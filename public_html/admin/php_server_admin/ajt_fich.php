...
if( isset($_FILES['fichier']) and isset($_POST['annee_etude'])and isset($_POST['module'])and isset($_POST['Type_fich']))
{
	...
	$fich_user_id=-1;
	$fich_ext=$_FILES['fichier']['type'];
	$time_fich=date('d-m-Y_H_i_s',time());

	if($fich_type=='cour'){$fich_path='/upload/cour/'.$time_fich.$fich_nom; $table='cour';}
	elseif($fich_type=='td' or $fich_type=='td_corr'){$fich_path='/upload/td/'.$time_fich.$fich_nom; $table='td';}
	elseif($fich_type=='tp' or $fich_type=='tp_corr'){$fich_path='/upload/tp/'.$time_fich.$fich_nom; $table='tp';}
	elseif($fich_type=='emd' or $fich_type=='emd_corr'){$fich_path='/upload/emd/'.$time_fich.$fich_nom; $table='emd';}
