...
include('cnx_db.php');
	...
	$fich_all=$bdd->query("SELECT * FROM  $table  WHERE  `cour_anne_univ` LIKE '%$fich_annee_univ%' AND  `cour_module` LIKE  '%$fich_module%' AND  `cour_nom` LIKE  '%$fich_nom%' AND `cour_type` LIKE  '%$fich_type%' ORDER BY  `cour_date_pub` DESC  LIMIT $debut , $nbr_p_page");
...
include('cnx_db.php');
	...
	foreach ($arr as &$table) {
		...
		$fich_all=$bdd->query("SELECT * FROM  $table  ORDER BY  `cour_date_pub` DESC  LIMIT $debut , $nbr_p_page");
